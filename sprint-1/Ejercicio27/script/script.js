function validHorizontalMatrix(matrix) {
  let counter = 0;
  let matrixCount = 0;
  for (let i = 0; i < matrix.length; i++) {
    matrixCount++;
    counter = 0;
    for (let x = 0; x < matrix[i].length; x++) {
      let actualArraySorted = [...matrix[i]].sort();
      if (actualArraySorted[x] === x + 1) {
        counter++;
      } else return false;
      if (i === 8 && counter === 9 && matrixCount === 9) {
        return true;
      }
    }
  }
  return false;
}

function buildNewArrayOfArraysWithPositions(matrix) {
  let arrayPosition = 0;
  let newMatrix = [];
  let newArray = [];

  for (let i = 0; i < matrix.length; i++) {
    for (let x = 0; x < matrix[i].length; x++) {
      newArray.push(matrix[i][arrayPosition]);
      if (i === 8) {
        newMatrix.push(newArray);
        arrayPosition++;
        newArray = [];
        i = -1;
        if (newMatrix.length === 9) {
          return validHorizontalMatrix(newMatrix);
        }
      }
      break;
    }
  }
}

function validarSudoku(matrix) {
  if (validHorizontalMatrix(matrix)) {
    return buildNewArrayOfArraysWithPositions(matrix);
  }
}

console.log(
  validarSudoku([
    [5, 3, 4, 6, 7, 8, 9, 1, 2],
    [6, 7, 2, 1, 9, 5, 3, 4, 8],
    [1, 9, 8, 3, 4, 2, 5, 6, 7],
    [8, 5, 9, 7, 6, 1, 4, 2, 3],
    [4, 2, 6, 8, 5, 3, 7, 9, 1],
    [7, 1, 3, 9, 2, 4, 8, 5, 6],
    [9, 6, 1, 5, 3, 7, 2, 8, 4],
    [2, 8, 7, 4, 1, 9, 6, 3, 5],
    [3, 4, 5, 2, 8, 6, 1, 7, 9],
  ])
);
